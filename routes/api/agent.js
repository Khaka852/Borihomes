...
router.get('/properties', async (req, res) => {
  const rows = await db.prepare('SELECT * FROM properties WHERE agent_id = ? ORDER BY created_at DESC').all(req.user.agentId);
  const results = [];
  for (const r of rows) {
    ...
    results.push(toPrivateProperty(r, images, priv, req.user.name, landlord));
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    const propResult = await db.prepare(`
...
router.post('/properties/:id/images', async (req, res) => {
  ...
  const maxRow = await db.prepare('SELECT MAX(display_order) as m FROM property_images WHERE property_id = ?').get(row.id);
  const maxOrder = maxRow.m || 0;
  await db.prepare('INSERT INTO property_images (property_id, image_url, image_type, display_order) VALUES (?,?,?,?)')
...
router.put('/enquiries/:id/status', async (req, res) => {
  const row = await db.prepare('SELECT * FROM enquiries WHERE id = ?').get(req.params.id);
  ...
  await db.prepare('UPDATE enquiries SET status = ? WHERE id = ?').run(status, row.id);
  res.json({ ok: true });
});
...
router.put('/inspections/:id/status', async (req, res) => {
  const row = await db.prepare('SELECT * FROM inspections WHERE id = ?').get(req.params.id);
  ...
  await db.prepare('UPDATE inspections SET status = ? WHERE id = ?').run(status, row.id);
  res.json({ ok: true });
});
